...
if($_SERVER['REQUEST_METHOD'] == 'POST' ) {
	...
	if($up->execute()){
		...
		$up_inv->execute();
		$up_inv->close();
		header('location:../extend/alertas.php?msj=Cliente actualizado&c=cli&p=in&t=success');
	} else {
		header('location:../extend/alertas.php?msj=No se pudo actualizar el cliente&c=cli&p=in&t=error');
	}
