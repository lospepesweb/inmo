      ...
      <div class="card-content">
          ...
          <div class="row">
            <div class="col s6">
              <select id="provincia" name="provincia" required="">
                <?php 
                $sel_prov = $con->prepare("SELECT idprovincia FROM provincias WHERE provincia = ? ");
                $sel_prov->bind_param('s', $f_prop['provincia']);
                $sel_prov->execute();
                $res_prov = $sel_prov->get_result();
                if ($f_prov = $res_prov->fetch_assoc()) {
                }
                ?>
                <option value="<?php echo $f_prov['idprovincia']?>"><?php echo $f_prop['provincia']?></option>
                <?php $sel_provincia = $con->prepare("SELECT * FROM provincias ");
                $sel_provincia->execute();
                $res_provincia = $sel_provincia->get_result();
                while ($f_provincia =$res_provincia->fetch_assoc()) {?>
                <option value="<?php echo $f_provincia['idprovicia'] ?>"><?php echo $f_provincia['provincia'] ?></option>
                <?php }
                $sel_provincia->close();
                 ?>
              </select>
            ...
            <div class="col s6">
              <div class="res_provincia">
                <input type="text" name="departamento" value="<?PHP echo $f_prop['departamento']?>" readonly>
              </div>
            </div>
          </div>
        <div class="row">
          <div class="col s6">
            ...
            <div class="input-field">
              <input type="text" name="barrio"  id="barrio" required onblur="may(this.value, this.id)" value="<?php echo $f_prop['barrio'] ?>" >
          ...
          <div class="col s12">
            <div class="input-field">
              <input type="text" name="mapa"  id="mapa" value="<?php echo $f_prop['mapa']?>">
        ...
        <div class="row">
          ...
          <div class="col s6">
            ...
            <div class="input-field">
              <input type="number" name="dormitorios"   id="dormitorios" value="<?php echo $f_prop['dormitorios'] ?>" >
